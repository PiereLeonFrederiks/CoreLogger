...
DOI: <[https://doi.org/10.5281/zenodo.21983775]>
...
- Layer types: **Normal**, **Disturbed / reworked**, **Sediment gap / missing**, **Anthropogenic** — detected automatically from Finds & Observations rather than set by hand
...
- Optional voice dictation: a guided step-by-step wizard walks through depth, soil type, modifiers, genesis, and gradational shift by speech, with a manual fallback at every step
...
- A guided **KA5 quick-entry** picker builds a code from a primary soil plus stacked subsoil components and genesis, or recalculates it from the current description
### Gradational & Banded Layers
...
- Separately, a layer can carry **two or more primary soils** present together (banded, speckled, mottled, or otherwise interbedded), each with its own subsoil components — distinct from the top→base gradational shift above
- Both are added to the description without overwriting existing free text, and redundant phrasings are suppressed (e.g. `L(l,t)` is shown as `L(t)`)
### Markers, Finds & Compaction Zones
 
- **Finds & Observations**: a tabbed panel (Sediment Conditions, Nature, Artefacts, Other) covering core disturbance, diagenesis, structure/fabric, samples, shells, charcoal, wood, ceramics, humic concentrations, roots, stones, and iron oxide — logged at a single depth or across a depth zone, and shown as labelled flags on the stratigraphic profile
- Find textures follow each layer's shape and are drawn semi-transparently, so the sediment grain stays visible underneath
- **Compaction zones**: flag corer-induced disturbance depths (compaction, extension, shearing, void, flow-in, etc.) with a note; displayed as distinct bands on the profile
- **Stratigraphic units**: group several layers into a broader named zone, shown as a bracket alongside the profile
- Auto-generated KA5 soil horizons for the whole profile, one click
### GPS & Coordinates
 
- One-tap **GPS capture** using the browser's Geolocation API, with a manual-entry fallback (including a paste-and-parse prompt) when GPS isn't available or permitted
...
- Optional **grain-size / weathering profile** column style, tapering each layer's width to its grain-size fraction, including across gradational and banded boundaries
- Boundary line style reflects recorded transition character (sharp, gradual, wavy)
...
| **Save Profile (JSON)**  | Full session state — layers, gradational/banded soils, markers, compaction, coordinates |
...
JSON is the lossless format and round-trips every field, including gradational shifts, banded soils, and finds. The TXT/CSV import reconstructs gradational shifts from the description and re-reads finds with their type and depth range, so an exported log re-imports faithfully.
